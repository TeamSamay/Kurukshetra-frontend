    ...
    <div className="min-h-screen w-full flex flex-col selection:bg-[#f8c858] selection:text-neutral-900 bg-[#f6f6f2]">
      ...
      <header className="w-full px-3 sm:px-6 lg:px-8 pt-4 pb-3 flex items-center justify-between gap-3 max-w-[1700px] mx-auto print:hidden">
        ...
        <div className="flex items-center gap-2.5 flex-shrink-0" ref={profileRef}>
          {/* Attacker Sandbox Button */}
          <button
            onClick={onOpenSandbox}
            className="px-3.5 py-2 rounded-full border border-neutral-300/80 bg-white/90 backdrop-blur-md shadow-xs hover:border-[#f8c858] hover:bg-neutral-50 transition-all flex items-center gap-2 cursor-pointer group text-xs font-bold text-neutral-900"
            title="Open Safe Interactive Attacker Terminal for Judges"
          >
            <Terminal className="w-3.5 h-3.5 text-[#f8c858]" />
            <span className="hidden sm:inline">Attacker Sandbox</span>
            <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
          </button>

          {/* Simulate Attack One-Click Button */}
          <button
            onClick={handleSimulate}
            disabled={simulating}
            className="px-3.5 py-2 rounded-full bg-neutral-900 hover:bg-neutral-800 text-white text-xs font-bold transition-all shadow-xs flex items-center gap-1.5 cursor-pointer disabled:opacity-60"
            title="Inject full simulated attack campaign"
          >
            {simulating ? (
              <Activity className="w-3.5 h-3.5 animate-spin text-[#f8c858]" />
            ) : (
              <Play className="w-3.5 h-3.5 fill-[#f8c858] text-[#f8c858]" />
            )}
            <span className="hidden md:inline">{simulating ? 'Simulating...' : 'Simulate'}</span>
          </button>
            ...
            {showProfileMenu && (
              <div className="absolute right-0 mt-2.5 w-80 rounded-2xl bg-white/95 backdrop-blur-xl border border-neutral-200/90 shadow-2xl z-[150] p-3 text-neutral-800 animate-in fade-in slide-in-from-top-2 duration-150">
                ...
                <div className="space-y-1">
                  ...
                  >
                    <div className="w-8 h-8 rounded-lg bg-neutral-900 text-[#f8c858] flex items-center justify-center flex-shrink-0">
                      <Terminal className="w-4 h-4" />
                    </div>
                    <div className="flex-1">
                      <span className="text-xs font-bold text-neutral-900">Attacker Sandbox</span>
                      <p className="text-[11px] text-neutral-500">Launch live honeypot terminal</p>
                    </div>
                  </button>
